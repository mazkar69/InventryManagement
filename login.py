...
class Login_System:
    def __init__ (self,root):
        ...
        self.root.title("Login System | Developed by AzKaR")
        ...
        lbl_employeId=Label(login_frame,text="Employe Id",font=("anadlus",15),bg="white",fg="#767171").place(x=50,y=100)
        txt_employeId=Entry(login_frame,font=("times new roman",15),textvariable=self.var_employeeId,bg="#ECECEC").place(x=50,y=140,width=250)

        lbl_pass=Label(login_frame,text="Password",font=("anadlus",15),bg="white",fg="#767171").place(x=50,y=200)
        txt_password=Entry(login_frame,font=("times new roman",15),bg="#ECECEC",textvariable=self.var_password,show="*").place(x=50,y=240,width=250)


        btn_login=Button(login_frame,text="Log In",font=("Arial Ronded MT Bold",15),activeforeground="white",cursor="hand2",bg="#00B0F0",fg="white",activebackground="#00B0F0",command=self.login).place(x=50,y=300,width=250,height=35)

        hr=Label(login_frame,bg="lightgrey").place(x=50,y=370,width=250,height=2)
        or_=Label(login_frame,text="Or",font=("times new roman",15,"bold"),bg="white",fg="lightgrey").place(x=150,y=355)

        btn_forget=Button(login_frame,text="Forget Password",font=("times new roman",13),bd=0,bg="white",fg="#00759E",activeforeground="white",activebackground="white",cursor="hand2",command=self.forgetPassword).place(x=100,y=390)
        ...
        lbl_reg=Label(registerFrame,text="Dont have an account ?",font=("times new roman",13),bg="white").place(x=40,y=20)
        btn_signup=Button(registerFrame,text="Sign Up",font=("times new roman",13,"bold"),bd=0,bg="white",fg="#00759E",activeforeground="white",activebackground="white",cursor="hand2").place(x=200,y=20)
    ...
    def login(self):
        if(self.var_password.get()=="" or self.var_employeeId.get()==""):
            ...
            cur=con.cursor()  
            try:
                cur.execute("select utype from employee where eid=? and pass=?",(self.var_employeeId.get(),self.var_password.get()))
                user=cur.fetchone()
                if user==None:
            ...
            try:
                ...
                if(email==None):
                    ...
                    if(chk=='f'):
                        ...
                        self.btn_reset=Button(self.forget_win,text="submit",font=("times new roman",15),bg="lightblue",cursor="hand2",activebackground="lightblue",command=self.sub_otp)
                        ...
                        self.btn_update=Button(self.forget_win,text="Update",font=("times new roman",15),bg="lightblue",cursor="hand2",activebackground="lightblue",state=DISABLED,command=self.update_pas)
